...
def load_data():
    try:
        ...
        st.error("movies.csv file not found. Please check the file path.")
    except Exception as e:
...
similarity = create_similarity_matrix(movies_data) if not movies_data.empty else None
    ...
    if movie_name:
        try:
            ...
            if movie_data and movie_data['Response'] == 'True':
                ...
                st.error("No movie found on OMDb API. Please try another name.")
        except Exception as e:
